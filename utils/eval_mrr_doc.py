...
import faiss
import numpy as np
from data.process_fn import dual_process_fn_doc
...
def combined_dist_eval(args, model, queries_path, passage_path, query_fn, psg_fn, topk_dev_qid_pid, ref_dict):
    ...
    for i, qid in enumerate(query_ids):
        ...
        ds, top_pids = get_topk_restricted(
            q_emb, psg_embs, pid_dict, psg_ids, pid_subset, 100)
...
def compute_mrr(D, I, qids, ref_dict):
    ...
    if is_first_worker():
        ...
        idx = np.argsort(D_merged, axis=1)[:, ::-1][:, :100]
        ...
        mrr_metrics = compute_metrics(ref_dict, candidate_dict,MaxMRRRank=100)
        mrr = mrr_metrics["MRR @100"]
        print(mrr)
    return mrr
